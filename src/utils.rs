...
pub fn combine_melds(division: &Division, calls: &Vec<Call>) -> Vec<Vec<Tile>> {
    let mut melds = division.melds.clone();
    melds.append(&mut calls.iter().map(|call| call.meld()).collect());
    melds
}

pub fn combine_melds_with_pair(division: &Division, calls: &Vec<Call>) -> Vec<Vec<Tile>> {
    let mut melds = combine_melds(division, calls);
    melds.push(division.pair.clone());
    melds
}

pub fn flatten_tiles(division: &Division, calls: &Vec<Call>) -> Vec<Tile> {
